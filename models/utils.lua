...
function utils.UnifInit(model)
   for k,v in pairs(model:findModules('nn.SpatialConvolution')) do
      local n = v.kW*v.kH*v.nOutputPlane
      v.weight:uniform(-math.sqrt(2/n),math.sqrt(2/n))
      if v.bias then v.bias:zero() end
   end
end
...
function utils.testModel(model, opt)
   model:float()
   local imageSize = opt  and opt.imageSize or {3,224,224}
   local input = torch.randn(1,table.unpack(imageSize)):type(model._type)
